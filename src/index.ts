			...
			<html lang="en">
				...
				<body>
					<div class="container">
						<div class="ipcard">
					        ...
					        <div class="whois-section" id="whois-section">
					            <h3>WHOIS Lookup</h3>
					            <input type="text" id="whois-input" placeholder="Search for IP or domain" />
					            <button id="whois-button">Lookup</button>
					            <pre id="whois-result" class="hidden"></pre>
					        </div>
				...
				<script nonce="${nonce}">
					document.addEventListener('DOMContentLoaded', () => {
			            ...
			            // Event listener for WHOIS lookup
			            const whoisButton = document.getElementById('whois-button');
			            whoisButton.addEventListener('click', async () => {
			                const whoisInput = document.getElementById('whois-input').value;
			                const whoisResult = document.getElementById('whois-result');
			                whoisResult.textContent = 'Fetching WHOIS data...';
			                whoisResult.classList.remove('hidden');

			                try {
			                    const response = await fetch('/whois/' + whoisInput);
			                    if (response.ok) {
			                        const data = await response.json();
			                        whoisResult.textContent = JSON.stringify(data, null, 2);
			                    } else {
			                        whoisResult.textContent = 'Error fetching WHOIS data';
			                    }
			                } catch (error) {
			                    whoisResult.textContent = 'Error fetching WHOIS data';
			                }
